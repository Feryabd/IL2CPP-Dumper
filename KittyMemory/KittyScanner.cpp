...
uintptr_t findInRange(const uintptr_t start, const uintptr_t end, const uint8_t *pattern, const char *mask)
{
    ...
    while (mask[first_fixed] == '?') {
        first_fixed++;
        if (first_fixed == mask_len) return 0;
    }

    const uint8_t target_byte = pattern[first_fixed];
    ...
    {
        const uint8_t *found_byte = static_cast<const uint8_t *>(memchr(cur + first_fixed, target_byte, (scan_end - cur) + 1));
        if (!found_byte)
            break;

        cur = found_byte - first_fixed;
...
ElfScanner::ElfScanner(IKittyMemOp *pMem, uintptr_t elfBase, const std::vector<KittyMemoryMap::ProcMap> &maps)
{
    _pMem = nullptr;
    _elfBase = 0;
    _ehdr = {};
    ...
    // verify address
    auto elfBaseMap = KittyMemoryMap::getAddressMap(_pMem->processID(), elfBase, maps);
    ...
    if (!KittyMemoryMap::getAddressMap(_pMem->processID(), _elfBase + _ehdr.e_phoff, maps).readable)
    {
    ...
    for (auto &phdr : _phdrs)
    {
        if (phdr.p_type == PT_DYNAMIC)
        {
            if (phdr.p_vaddr == 0 || phdr.p_memsz == 0)
                break;
            if (!KittyMemoryMap::getAddressMap(_pMem->processID(), _loadBias + phdr.p_vaddr, maps).readable)
                break;
            if (!KittyMemoryMap::getAddressMap(_pMem->processID(), _loadBias + phdr.p_vaddr + (phdr.p_memsz - 1), maps)
                     .readable)
                break;
    ...
    auto fix_table_address = [&](uintptr_t &table_addr) {
        ...
        if (!KittyMemoryMap::getAddressMap(_pMem->processID(), table_addr, maps).readable)
            table_addr = 0;
    };
...
#ifdef __ANDROID__
ElfScanner::ElfScanner(IKittyMemOp *pMem, const kitty_soinfo_t &soinfo, const std::vector<KittyMemoryMap::ProcMap> &maps)
{
    _pMem = nullptr;
    _elfBase = 0;
    _ehdr = {};
    ...
    // fix for linker
    if (_elfBase == 0)
        _elfBase = KittyMemoryMap::getAddressMap(_pMem->processID(), soinfo.bias, maps).startAddress;
    if (_elfBase == 0)
        _elfBase = KittyMemoryMap::getAddressMap(_pMem->processID(), soinfo.phdr, maps).startAddress;
    if (_elfBase == 0)
        _elfBase = KittyMemoryMap::getAddressMap(_pMem->processID(), soinfo.dyn, maps).startAddress;
    if (_elfBase == 0)
        _elfBase = KittyMemoryMap::getAddressMap(_pMem->processID(), soinfo.symtab, maps).startAddress;
    if (_elfBase == 0)
        _elfBase = KittyMemoryMap::getAddressMap(_pMem->processID(), soinfo.strtab, maps).startAddress;

    // verify address
    auto elfBaseMap = KittyMemoryMap::getAddressMap(_pMem->processID(), _elfBase, maps);
    ...
    auto phdrMap = KittyMemoryMap::getAddressMap(_pMem->processID(), _phdr, maps);
    ...
    if (!isLinker)
    {
        auto dynMap = KittyMemoryMap::getAddressMap(_pMem->processID(), _dynamic, maps);
    ...
    // fix for ldplayer
    auto biasMap = KittyMemoryMap::getAddressMap(_pMem->processID(), _loadBias, maps);
    ...
    for (auto &phdr : _phdrs)
    {
        if (phdr.p_type == PT_DYNAMIC)
        {
            ...
            if (_dynamic == 0 || phdr.p_memsz == 0)
                break;
            if (!KittyMemoryMap::getAddressMap(_pMem->processID(), _dynamic, maps).readable)
                break;
            if (!KittyMemoryMap::getAddressMap(_pMem->processID(), _dynamic + (phdr.p_memsz - 1), maps).readable)
                break;
...
ElfScanner &ElfScannerMgr::getProgramElf()
{
    ...
    if (!_programElf.isValid() || !_programElf.dynamic())
    {
        ...
        const auto allMaps = KittyMemoryMap::getAllMaps(_pMem->processID());
        std::vector<KittyMemoryMap::ProcMap> exeMaps;
        ...
        // Make sure to always get native one
        {
            ...
            exeMaps = KittyMemoryMap::getMaps(_pMem->processID(),
                                             KittyMemoryMap::EProcMapFilter::Equal,
                                             binDir + "/" + exeName,
                                             allMaps);
            if (exeMaps.empty())
            {
...
#else
        exeMaps = KittyMemoryMap::getMaps(_pMem->processID(), KittyMemoryMap::EProcMapFilter::Equal, exePath, allMaps);
#endif
...
#endif
{
    ...
    auto maps = KittyMemoryMap::getAllMaps(_pMem->processID());
    if (maps.empty())
    {
    ...
    {
        if (it.first && !KittyMemoryMap::getAddressMap(_pMem->processID(), it.first, maps).readable)
        {
...
uintptr_t ElfScannerMgr::findRemoteSymbol(const std::string &local_sym_name, uintptr_t local_sym_addr)
{
    ...
    ElfScanner r_lib{};
    KittyMemoryMap::ProcMap l_lib{};

    l_lib = KittyMemoryMap::getAddressMap(getpid(), local_sym_addr);
    if (l_lib.isValid())
...
bool LinkerScannerMgr::init()
{
    ...
    auto allMaps = KittyMemoryMap::getAllMaps(_pMem->processID());
    ...
    for (size_t i = 0; i < si_buf.size(); i += sizeof(uintptr_t))
    {
        ...
        auto tmp_map = KittyMemoryMap::getAddressMap(_pMem->processID(), possible_base, allMaps);
    ...
    for (size_t i = 0; i < si_buf.size(); i += sizeof(uintptr_t))
    {
        ...
        if (!_soinfo_offsets.size &&
            (value == si_elf.loadSize() ||
             value ==
                 (si_elf.loadSize() + KittyMemoryMap::getAddressMap(_pMem->processID(), si_elf.end(), allMaps).length)))
        {
    ...
    for (size_t i = 0; i < solist_buf.size(); i += sizeof(uintptr_t))
    {
        ...
        if (!KittyMemoryMap::getAddressMap(_pMem->processID(), possible_next + _soinfo_offsets.base, allMaps).readable)
            continue;
        ...
        if (tmp_elf.isValid())
        {
            if (!KittyMemoryMap::getAddressMap(_pMem->processID(), possible_next + _soinfo_offsets.size, allMaps)
                     .readable)
                continue;
            ...
            if (possible_size == tmp_elf.loadSize() ||
                possible_size == (tmp_elf.loadSize() +
                                  KittyMemoryMap::getAddressMap(_pMem->processID(), tmp_elf.end(), allMaps).length))
            {
...
std::vector<kitty_soinfo_t> LinkerScannerMgr::allSoInfo() const
{
    ...
    while (si && KittyMemoryMap::getAddressMap(_pMem->processID(), si, maps).readable)
    {
...
kitty_soinfo_t LinkerScannerMgr::infoFromSoInfo_(uintptr_t si, const std::vector<KittyMemoryMap::ProcMap> &maps) const
{
    kitty_soinfo_t info{};
    ...
    auto si_map = KittyMemoryMap::getAddressMap(_pMem->processID(), start_map_addr, maps);
    if (si_map.isValid())
    {
    ...
    else // Houdini
    {
        ...
        for (auto &sohead : soheads)
        {
            ...
            for (size_t i = 0; i < si_buf.size(); i += sizeof(uintptr_t))
            {
                ...
                if (!KittyMemoryMap::getAddressMap(_pMem->processID(), possible_next, maps).readable)
                    continue;
                ...
                for (size_t j = 0; j < si_buf_inner.size(); j += sizeof(uintptr_t))
                {
                    ...
                    auto tmp_map = KittyMemoryMap::getAddressMap(_pMem->processID(), possible_base, maps);
                ...
                for (size_t j = 0; j < si_buf_inner.size(); j += sizeof(uintptr_t))
                {
                    ...
                    if (!sohead.offsets.size &&
                        (value == si_elf.loadSize() ||
                         value == (si_elf.loadSize() +
                                   KittyMemoryMap::getAddressMap(_pMem->processID(), si_elf.end(), maps).length)))
                    {
...
kitty_soinfo_t NativeBridgeScannerMgr::infoFromSoInfo_(uintptr_t si,
                                                       const std::vector<KittyMemoryMap::ProcMap> &maps) const
{
    kitty_soinfo_t info{};
