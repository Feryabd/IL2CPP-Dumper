...
#define KITTY_LOGW(fmt, ...) os_log(OS_LOG_DEFAULT, "W " KITTY_LOG_TAG ": " fmt, ##__VA_ARGS__)
...
namespace KittyUtils
{
    ...
    namespace Android
    {
         ...
         */
        inline std::string getAppExternalDataDir(const std::string &pkg = "")
        {
            return getExternalStorage() + "/Android/data/" + (pkg.empty() ? getprogname() : pkg);
        }
         ...
         */
        inline std::string getAppExternalFilesDir(const std::string &pkg = "")
        {
            return getAppExternalDataDir(pkg) + "/files";
        }
